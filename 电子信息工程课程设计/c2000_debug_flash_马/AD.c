...
int xn,yn, zn,Avg,last_zn=32767;
unsigned long q;
unsigned long led_light;
int Freq=0, Period=0, Value=0;//频率计数变量
long int  Freq_time=0;
int zn_max=0,zn_min=32767;
unsigned int flag_zn=1, y_index = 0;
...
/*		else
	...
	while(can!=6)
    {
		...
		Value =( (zn_max - zn_min)*14.01869-10.2804);
		

		//GUILCD_writeCharStr(0x03,0x0A,zn_max%10, FALSE);
		//GUILCD_writeCharStr(0x03,0x09,71,FALSE)
		GUILCD_writeCharStr(0x04,0x04,zn_max/10%10, FALSE);
		GUILCD_writeCharStr(0x04,0x03,zn_max/100%10, FALSE);
		GUILCD_writeCharStr(0x04,0x02,zn_max/1000%10, FALSE);
		GUILCD_writeCharStr(0x04,0x01,zn_max/10000%10, FALSE);
	...
	//	GUILCD_writeCharStr(0x06, 0x0A, zn_threshold%10, FALSE);
	//	GUILCD_writeCharStr(0x06, 0x09, 71, FALSE);
		GUILCD_writeCharStr(0x05, 0x04,  zn_threshold/10%10, FALSE);
		GUILCD_writeCharStr(0x05, 0x03,  zn_threshold/100%10, FALSE);
		GUILCD_writeCharStr(0x05, 0x02,  zn_threshold/1000%10, FALSE);
		GUILCD_writeCharStr(0x05, 0x01,  zn_threshold/10000%10, FALSE);
...
void AD_DA()
{
	...
	zn=AdcRegs.ADCRESULT0>>4;
	GpioDataRegs.GPBDAT.bit.GPIOB3 = 0;
	* DAOUT=xn;
	if (zn_max<zn) zn_max=zn;
	if (zn_min>zn) zn_min=zn;

	/*************测频率***************/
	zn_threshold=zn_min+(zn_max-zn_min)*0.8;
	if ( last_zn < zn_threshold && zn>=zn_threshold )
	{
		flag_zn++;
		if(flag_zn>10)
		{
...
/*	if (Freq_time % 20==0){
		...
		plot((zn-(zn_min-100))*16/ (zn_max-zn_min), y_index);
	}
	//plot((zn-zn_min)*16 / Value, y_index);
	*/
}
