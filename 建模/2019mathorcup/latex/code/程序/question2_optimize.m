...
%--------------------plot pictures-------------------%
figure(1);
    plot(id, historical_income_rate_simulation_1','b.' );
    hold on
    plot(id, historical_income_rate_Actual, 'r.' );
    plot(id, relative_error_1,'g.');
    axis([-10 220 -0.1 1.1]);
    title(['Simulation diagram-',element, '-Without optimization']);
...
figure(2);
    ...
    hold off
figure(3);
    plot(id, relative_error_1','b+' );
    hold on
    plot(id, relative_error_2,'r^');
    plot([-10 220], [mean(relative_error_1), mean(relative_error_1)], 'b-');
    plot([-10 220], [mean(relative_error_2), mean(relative_error_2)], 'r-');
    axis([-10 220 -0.01 0.10]);
    title(['Simulation diagram-',element, '-relative error']);
    xlabel('Sample number');
    ylabel('percentage');
    legend({'Relative error-without optimization','Relative error-optimized', 'Relative error average 1', 'Relative error average 2'},'Location', 'NorthEastOutside');
    hold off
mean(relative_error_1)
