...
% 自变量维度
m=4;   % 自变量：角度alpha，lamda钢筋位置，h板厚度，n板左右各分多少木条
...
[alpha1,alpha2]=alpha_search(n1,n2);



for k=130 % 最外部为了批量得到数据的大循环
%% 样本初始化
max_pop=[];
pop=zeros(pop_num,m);
pop(:,3)=rand(pop_num,1)*1+3;
pop(:,1)=(alpha2-alpha1)*rand(pop_num,1)+alpha1;
...
for j=1:pop_num
fitness(j)=fit_and_select2(pop(j,1),pop(j,1),pop(j,3),pop(j,2),pop(j,4),pop(j,4));    
end
...
if p<p_to_c
    pop(j,3)=rand()*1+3;
    pop(j,1)=(alpha2-alpha1)*rand()+alpha1;
...
while 1
   ...
   f_s=zeros(1,11);
   for s=1:5
       [~,f_s(6+s)]= B3okornot(D0,H0,nx,hx,lamda,lamda,(theta+0.01*s),(theta+0.01*s));
   ...
   for s=1:11
       if s~=1&&s~=11
          if f_s(s)<f_s(s+1)&&f_s(s)<f_s(s-1)
          flag_s =1;
          break
          end
       end
...
toc
 ...
 xlswrite('C:\Users\bingjilin1995ydd\Desktop\M文件整理\data_harvest\note2.xlsx',max_pop,'note2',['B',num2str(k),':E',num2str(k)]);
