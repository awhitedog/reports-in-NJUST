...
clc,clear
close all
% global nx l_d lamda hx d theta
% 初期参数决定
H0=70;
D0=80;
pop_num=80;   % 种群个数
...
% 自变量维度
m=4;   % 自变量：L 板材总长，lamda钢筋位置，h板厚度，n板左右各分多少木条



for k=505:540  % 最外部为了批量得到数据的大循环
%% 样本初始化
max_pop=[];
pop=zeros(pop_num,m);
pop(:,3)=rand(pop_num,1)*1+3;
pop(:,1)=150+randi([5 12],[pop_num,1])+rand();
pop(:,2)=randi([20,30],[pop_num,1]);
...
for j=1:pop_num
fitness(j)=fit_and_select(pop(j,1),pop(j,3),pop(j,2),pop(j,4));    
end
...
if p<p_to_c
    pop(j,3)=rand()*1+3;
    pop(j,1)=150+randi([5 12],1)+rand();
    pop(j,2)=randi([20,30],1);
    pop(j,4)=(rand()*0.2+0.4);   %变异：整体随机改变
end
end
...
while 1
    ...
    f_s=zeros(1,5);
    for s=1:5
        theta=asin((H0-hx)/li(1,l_d+s-3,d,D0));
        f_s(s)=li(1,l_d+s-3,d,D0)*li(1,l_d+s-3,d,D0)/2*sin(theta);
    for g=2:floor(nx/2)
    f_s(s)=f_s(s)+li(g,l_d+s-3,d,D0)*hi(g,l_d+s-3,H0,hx,lamda,D0,theta,d);       % 以L（木板长）为自变量的符号方程，其中省略了厚度d，密度tho，重力加速度
    end
    if floor(nx/2)~=nx/2
    f_s(s)=f_s(s)+li(ceil(nx/2),l_d+s-3,d,D0)*hi(ceil(nx/2),l_d+s-3,H0,hx,lamda,D0,theta,d)/2; % 基数时中间的木条是与圆相切的，质量取一半研究
    end
    end
    
    if (f_s(3)<f_s(4)&&f_s(2)>f_s(3))||(f_s(2)<f_s(1)&&f_s(3)>f_s(2))||(f_s(4)<f_s(5)&&f_s(3)>f_s(4))
    break
    end
...
for j=1:pop_num
 ...
 %% 每做一次自动写入数据
 xlswrite('C:\Users\bingjilin1995ydd\Desktop\M文件整理\data_harvest\note.xlsx',1/max_fitness,'max_note',['A',num2str(k)]);
 xlswrite('C:\Users\bingjilin1995ydd\Desktop\M文件整理\data_harvest\note.xlsx',max_pop,'max_note',['B',num2str(k),':E',num2str(k)]);
